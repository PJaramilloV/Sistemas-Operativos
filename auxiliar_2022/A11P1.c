// Aux de programar driver

// para instalar driver usar: 
/*  $ sudo mknod /dev/vigia c 61 0
    $ sudo chmod a+rw /dev/vigia
    $ sudo insmod vigia.ko
    $ sudo dmesg | grep vigia | vigia
...
/* Declaration of vigia.c functions */
int vigia_open(struct inode *inode, struct file *filp);
int vigia_release(struct inode *inode, struct file *filp);
ssize_t vigia_read(struct file *filp, char *buf, size_t count, loff_t *f_pos);
ssize_t vigia_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos);
...
// static es que solo se usa en este codigo
...
//      registrar numero major e iniciar variables globales
