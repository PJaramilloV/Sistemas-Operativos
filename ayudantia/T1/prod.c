...
typedef struct {
    int* a; // Puntero de un array
    int i, j; // Indices
    int p; // Numero de threads
    BigNum* res; // Resultado de las multiplicaciones
} Args;

void *thread_function(void *ptr){
    Args *arg = (Args*) ptr; // Casteo
    arg->res = parArrayProd(arg->a, arg->i, arg->j, arg->p); // Guardamos el resultado en la estructura
    return NULL;
}
...
BigNum *parArrayProd(int a[], int i, int j, int p) {
  ...
  if (i==j){
    ...
    if (p==1) {
      ...
      // Calculo pivote
      int h= (i+j)/2;
      // Creo id del p_thread
      pthread_t pid;
      // Creo estructura de los argumentos para el thread
      Args args = {&(a[0]), i, h, p-(p/2)};
      // Paralelizo trabajo en el thread creado
      pthread_create(&pid, NULL, thread_function, &args);
      // Trabajo en el thread actual
      BigNum *right= parArrayProd(a, h+1, j, p/2);
      // Entierro el thread paralelo
      pthread_join(pid,NULL);
      // Calculo el producto de ambos threads
      BigNum *prod= bigMul(args.res, right); // Multiplicacion de BigNum's       
      // Hay que liberar la memoria ocupada por los BigNum's
      freeBigNum(right);
      freeBigNum(args.res);
      return prod;
    }
